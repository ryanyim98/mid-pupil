...
for (sub in unique(fmri3_pupil_data$subject)){
  ...
  for (t in unique(my.df.temp$trial)){
    ...
    my.df.temp.trial <- my.df.temp.trial %>% 
      mutate(luminance_deriv = luminance - lag(luminance),
             luminance_deriv = ifelse(is.na(luminance_deriv),0,luminance_deriv))
    ...
    my.df.temp.trial$convolved_new_deriv <- convolve(my.df.temp.trial$luminance_deriv, rev(gamma_pdf_new), type = "open")[1:(nrow(my.df.temp.trial))]
