...
cd('~/Desktop/VRMID-analysis/mid-pupil/data/fmri3/raw');
...
for p = 1:nsubjects
    if sum(~isnan(alldata.subjectdata(p).Physio.LeftPDil.data.islandRemove)) %js b3
        makefigure(30,10);
        plot(alldata.subjectdata(p).Physio.LeftPDil.data.islandRemove);
...
subplot(2,2,2);
plot(ind/sr,RAW_vis2l,'lineWidth',3); hold on;
plot(ind/sr,RAW_vis3l,'lineWidth',3); hold on; 
...
plot(ind/sr,RAW_vis5l,'lineWidth',3); hold on;
...
print('~/Desktop/midaffemo/analyses/pupil/example_pupil.tiff','-dtiff','-r300');
...
% this takes a while
writetable(TdataOut, '~/Desktop/midaffemo/analyses/pupil/derivatives/pupillometry.csv');
clc;
