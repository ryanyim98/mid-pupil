...
```{r}
data_pupil <- read_csv("~/Desktop/VRMID-analysis/vrmid-pupillometry/data/study2/pupil_cleaned.csv")
...
ggplot(data_pupil%>%
       filter(trial == 2, subject == "ab230613"),
...
ggplot(data_pupil_summary_motion, aes(x = sample_in_trial_t,
  ...
  labs(title = "anticipation phase",
       x = "time (s)",
       y = "z-scored pupil size")
ggsave("../../figures/study2_fig2_motion.png",width = 5, height = 5, units = "in",dpi = 300)
```
...
write_csv(data_lm_anti,"~/Desktop/VRMID-analysis/vrmid-pupillometry/data/study2/data_lm_anticipation_cleaned.csv")
```
...
data_lm_outcome <-  data_pupil%>%
  mutate(pupil_window = ifelse(probe == 2 & sample_in_trial_t <= 8 + 2 & 
                               ...
                               ifelse(sample_in_trial_t <= anticipation_window[2] & 
                                      ...
                                      "ant",NA))))%>%
  filter(pupil_window %in% c("out","ant"),
         rating_type == "affect")%>% #probe == "out"
  ungroup()%>%
...
write_csv(data_lm_outcome,"~/Desktop/VRMID-analysis/vrmid-pupillometry/data/study2/data_lm_outcome_cleaned.csv")
```


```{r}
ind_pupil_out_sub <- data_pupil%>%
  filter(ifelse(probe == 1, sample_in_trial_t <= 16 + 3.5 & sample_in_trial_t > 16 + 1.5,
...
pb2
ggsave("../../figures/study2_axisrot_cleaned.png",width = 5, height = 4, units = "in",dpi = 300)
```
...
write.csv(regression_coefs_ant_nobc, "../../data/study2/ant_regression_coefs_nobc_cleaned.csv")
```
...
for (t in 1:20) {
  ...
  filter(probe == 2,Time_sec == t, sample_in_sec == s)
    ...
    if (nrow(df_out_temp) > 0){
        df_out_temp <- lmer(scale(arousal_scaled) ~ scale(cleaned_pupilDiameter) + (1|subject) + (1|condition),df_out_temp)
...
regression_coefs_out_nobc <- regression_coefs_out_nobc%>%
  mutate(sample_point = row_number(),
         p_val = (`Pr(>|t|)` < .001)) %>% 
  filter(sample_point <= 2400)

write.csv(regression_coefs_out_nobc,"../../data/study2/ant_regression_coefs_nobc_cleaned.csv")
regression_coefs_out_nobc <- read_csv("../../data/study2/ant_regression_coefs_nobc_cleaned.csv")
```
...
pp1/pp2
ggsave("../../figures/study2_corrpeaks_cleaned.png",width = 6, height = 10, units = "in",dpi = 300)
```
