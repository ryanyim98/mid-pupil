...
```{r}
data_pupil <- read_csv("/Users/rh/Desktop/VRMID-analysis/data/study2/pupillometry_baselineCorrected.csv")%>%
...
data_pupil <- data_pupil%>%
  mutate(axis_affect = ifelse(probe == "anti",
                df_axes2$cos[1] * valence_scaled + df_axes2$sin[1] * arousal_scaled,
                ifelse(probe == "out",
                       df_axes2$cos[2] * valence_scaled + df_axes2$sin[2] * arousal_scaled,NA)))
```
...
ant_bc_pupil$axis <- df_axes2$cos[1] * ant_bc_pupil$valence_scaled + df_axes2$sin[1] * ant_bc_pupil$arousal_scaled
...
ggsave("../../poster/fig11B.png",width = 5, height = 2, units = "in",dpi = 300)
```
#no bc bit
```{r}
p4a <- ggplot()+
