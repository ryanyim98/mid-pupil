...
theme_set(theme_bw() + #set the theme 
            theme(text = element_text(family = "Helvetica",size = 8))) #set the default text size
...
```{r}
my.df <- read.csv("../../data/study2/physio_1sec.csv")
...
my.df$Time_str <- times(ifelse(chron::hours(my.df$Time_str) < 7, times(my.df$Time_str + "12:00:00"),
                           my.df$Time_str))
...
my.df <- my.df%>%
  ...
  mutate(Time_str_start = Time_str[1],
         ...
         Time_sec = chron::seconds(Time_in_trial_sec))
...
my.df <- my.df%>%
  ...
  rename(ITI_duration = ITI.Duration)
```
...
my.df_rotated$trial.type <- factor(my.df_rotated$trial.type, levels = c("-$5","-$1","-$0","+$0","+$1","+$5"))
...
write_csv(dfOut,"/Users/yanyan/Desktop/VRMID-analysis/vrmid-pupillometry/data/study2/per_second_data.csv")
```

```{r}
summary(lmer(PosA_scaled ~ NegA_scaled + (1 + NegA_scaled|Subject),my.df_rotated))

p1 <- ggplot(my.df_beh%>%filter(probe == "anti"), aes(x = trial.type, y = arousal_scaled))+
...
p2 <- ggplot(my.df_beh%>%filter(probe == "anti"), aes(x = trial.type, y = valence_scaled))+
  stat_summary(fun.data = "mean_cl_boot",geom = "pointrange",
               size = 0.3)+
  labs(title = "valence",
       x = "cue value")+
...
```{r}
p3 <- ggplot(my.df_beh%>%filter(probe == "out"), aes(x = trial.type, y = arousal_scaled,
                                                      color = reaction.outcome,
                                                     shape = reaction.outcome))+
  stat_summary(fun.data = "mean_cl_boot",geom = "pointrange",
               size = 0.3)+
...
p4 <- ggplot(my.df_beh%>%filter(probe == "out"), aes(x = trial.type, y = valence_scaled,
                                                      color = reaction.outcome,
                                                     shape = reaction.outcome))+
  stat_summary(fun.data = "mean_cl_boot",geom = "pointrange",
               size = 0.3)+
...
ggsave("../../poster/fig3.png",width = 7, height = 2.5, units = "in",dpi = 300)
```
...
df <- my.df_physio_1hz%>%
  ...
  group_by(Subject, reaction.outcome,trial,probe, last_ITI,valence,trial.type,condition, arousal, ITI_duration,PosA_scaled,NegA_scaled)%>%
...
lm8 <- lmer(arousal ~ condition * trial.type + (1|Subject) + (1|last_ITI),df%>%filter(probe == "anti", condition %in% c("small","large")))
summary(lm8)
plot_model(lm8, type = "pred", terms = c("trial.type","condition"))+
  geom_line()+
  scale_color_manual(values = purpleOrange_palette2)

lm8 <- lmer(valence ~ condition * trial.type + (1|Subject) + (1|last_ITI),df%>%filter(probe == "anti", condition %in% c("small","large")))
summary(lm8)
plot_model(lm8, type = "pred", terms = c("trial.type","condition"))+
  geom_line()+
  scale_color_manual(values = purpleOrange_palette2)
...
```{r}
lm7b <- lmer(arousal ~ condition * trial.type + (1|Subject),
             df%>%filter(probe == "anti",condition %in% c("small","large")))
summary(lm7b)
anova(lm7b)
p1 <- plot_model(lm7b, type = "pred", terms = c("trial.type","condition"))+
  geom_line()+
  scale_color_manual(values = purpleOrange_palette2)

lm7c <- lmer(valence ~ condition * trial.type + (1|Subject),
             df%>%filter(probe == "anti",condition %in% c("small","large")))
                         # trial.type %in% c("-$5","+$5")))
summary(lm7c)
anova(lm7c)
p2 <- plot_model(lm7c, type = "pred", terms = c("trial.type","condition"))+
  geom_line()+
  scale_color_manual(values = purpleOrange_palette2)

lm7d <- lmer(pupil_size_scaled ~ condition * trial.type + (1|Subject) + (1|last_ITI),
             df%>%filter(condition %in% c("small","large")))
summary(lm7d)
anova(lm7d)
p3 <- plot_model(lm7d, type = "pred", terms = c("trial.type","condition"))+
  geom_line()+
  scale_color_manual(values = purpleOrange_palette2)

p1+p2+p3
ggsave("../../poster/fig_prereg2.png",width = 9, height = 2.5, units = "in",dpi = 300)
```
# affect trajectory
## no rotation
```{r}
...
aff_traj_avg_out_noRotat <- my.df_beh%>%
  ungroup()%>%
  filter(probe == "out")%>%
  group_by(trial.type,probe, reaction.outcome)%>%
  summarise(valence_mean = mean(valence, na.rm = T),
            arousal_mean = mean(arousal, na.rm = T),
            valence_sd = sd(valence, na.rm = T)/sqrt(30),
            arousal_sd = sd(arousal, na.rm = T)/sqrt(30))%>%
  mutate(report_type = reaction.outcome)%>%
  select(report_type,trial.type,valence_mean:arousal_sd)
...
aff_traj_avg_out_wRotat_5byMotion <- my.df_beh%>%
  ungroup()%>%
  filter(trial.type == "+$5")%>%
  group_by(probe,reaction.outcome,condition)%>%
  summarise_at(vars(PosA_scaled, NegA_scaled),
               ~mean(.x, na.rm = TRUE))%>%
  pivot_wider(names_from = probe, values_from = c(PosA_scaled,NegA_scaled))%>%
  rename(report_type =reaction.outcome)%>%
