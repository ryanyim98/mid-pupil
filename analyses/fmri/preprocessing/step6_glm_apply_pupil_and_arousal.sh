#!/bin/bash
#glm55 is the model with both parametric regressors for arousal ratings and pupil size
...
do
	...
	cd $DATA_DIR/subjects/$SUBJECT/func_proc_MIDaffemo
		...
		do
		    ...
		    3dDeconvolve -overwrite -float -input pp_MIDaffemo_b${KERNEL}_orig.nii.gz -nfirst 0 -num_stimts 16 -xjpeg Xmat -polort 2 -concat '1D: 0 245 490 735' \
		        ...
		        -stim_file 8 roi_ts/MIDaffemo_b${KERNEL}_csf_ants.1D -stim_base 8 -stim_label 8 csf \
						...
						-stim_file 16 regs/pupil_scaled_c.1D -stim_label 16 pupil_scaled_convolved \
		        -tout -fout -rout -bucket $OUT_DIR/${SUBJECT}_glm55_b${KERNEL}

		    3dmerge -doall -1zscore -prefix $OUT_DIR/${SUBJECT}_glm55_z_b${KERNEL} $OUT_DIR/${SUBJECT}_glm55_b${KERNEL}+orig

				3dAFNItoNIFTI -overwrite -prefix $OUT_DIR/${SUBJECT}_glm55_b${KERNEL}_orig.nii.gz $OUT_DIR/${SUBJECT}_glm55_b${KERNEL}+orig
				3dAFNItoNIFTI -overwrite -prefix $OUT_DIR/${SUBJECT}_glm55_z_b${KERNEL}_orig.nii.gz $OUT_DIR/${SUBJECT}_glm55_z_b${KERNEL}+orig
				...
				3dTcat -overwrite -prefix $temp_vol $OUT_DIR/${SUBJECT}_glm55_b${KERNEL}_orig.nii.gz[0]
				# Replace the first volume of the output dataset with the extracted volume
				3dTcat -overwrite -prefix $temp_output $temp_vol $OUT_DIR/${SUBJECT}_glm55_z_b${KERNEL}_orig.nii.gz[1..$]
				# do some cleaning
				mv $temp_output $OUT_DIR/${SUBJECT}_glm55_z_b${KERNEL}_orig.nii.gz
				rm $temp_vol

				rm $OUT_DIR/${SUBJECT}_glm55_b${KERNEL}+orig.*
				rm $OUT_DIR/${SUBJECT}_glm55_z_b${KERNEL}+orig.*
				...
				3dTcat -overwrite -prefix $OUT_DIR/${SUBJECT}_glm55_z_b${KERNEL}_IND_orig.nii.gz $OUT_DIR/${SUBJECT}_glm55_z_b${KERNEL}_orig.nii.gz[0..$]

				### apply the part2mni transformation
				flirt -in $OUT_DIR/${SUBJECT}_glm55_z_b${KERNEL}_IND_orig.nii.gz -init ../anat_proc/xfs/func2mni_ants.mat -applyxfm \
						-out $OUT_DIR/${SUBJECT}_glm55_z_b${KERNEL}_mni.nii.gz -paddingsize 0.0 -interp trilinear \
