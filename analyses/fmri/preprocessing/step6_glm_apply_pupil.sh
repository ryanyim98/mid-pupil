#!/bin/bash
#glm33b is the model with pupil size modulation (convolved with HRF), with regressors of reward stages
	...
	cd $DATA_DIR/subjects/$SUBJECT/func_proc_MIDaffemo
		...
		do
		    ...
		    3dDeconvolve -overwrite -float -input pp_MIDaffemo_b${KERNEL}_orig.nii.gz -nfirst 0 -num_stimts 15 -xjpeg Xmat -polort 2  -concat '1D: 0 245 490 735' \
		        ...
		        -tout -fout -rout -bucket $OUT_DIR/${SUBJECT}_glm33b_b${KERNEL}

		    3dmerge -doall -1zscore -prefix $OUT_DIR/${SUBJECT}_glm33b_z_b${KERNEL} $OUT_DIR/${SUBJECT}_glm33b_b${KERNEL}+orig

				3dAFNItoNIFTI -overwrite -prefix $OUT_DIR/${SUBJECT}_glm33b_b${KERNEL}_orig.nii.gz $OUT_DIR/${SUBJECT}_glm33b_b${KERNEL}+orig
		    3dAFNItoNIFTI -overwrite -prefix $OUT_DIR/${SUBJECT}_glm33b_z_b${KERNEL}_orig.nii.gz $OUT_DIR/${SUBJECT}_glm33b_z_b${KERNEL}+orig
				...
				3dTcat -overwrite -prefix $temp_vol $OUT_DIR/${SUBJECT}_glm33b_b${KERNEL}_orig.nii.gz[0]
				# Replace the first volume of the output dataset with the extracted volume
				3dTcat -overwrite -prefix $temp_output $temp_vol $OUT_DIR/${SUBJECT}_glm33b_z_b${KERNEL}_orig.nii.gz[1..$]
				# do some cleaning
				mv $temp_output $OUT_DIR/${SUBJECT}_glm33b_z_b${KERNEL}_orig.nii.gz
				rm $temp_vol

				rm $OUT_DIR/${SUBJECT}_glm33b_b${KERNEL}+orig.*
