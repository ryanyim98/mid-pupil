...
summary_data <- fmri3_pupil_data %>% 
  ...
  summarise(luminance = mean(luminance),
            convolved_orig=mean(convolved_orig),
            convolved_new = mean(convolved_new),
            convolved_orig_deriv = mean(convolved_orig_deriv)) %>% 
  filter(cue_value == "+$5")

ggplot()+
  geom_line(data=summary_data,aes(x = sample_in_trial_t, y = -luminance,
            linetype = cue_size), linewidth = 0.5, color = 'skyblue')+
  geom_line(data=summary_data,aes(x = sample_in_trial_t+0.1, y = -convolved_orig,
             linetype = cue_size), linewidth = 0.5, color = 'salmon')+
    geom_line(data=summary_data,aes(x = sample_in_trial_t+0.2, y = -convolved_new,
            linetype = cue_size), linewidth = 0.5, color = 'pink')+
      # geom_line(data=summary_data,aes(x = sample_in_trial_t, y = convolved_orig_deriv,
      #       linetype = cue_size), linewidth = 0.5, color = 'orange')+
...
summary(l1<-lmer(scale(pupilDiameter_bc_scaled) ~ scale(luminance) + (1|subject),fmri3_pupil_data))
summary(l2<-lmer(scale(pupilDiameter_bc_scaled) ~ scale(luminance_lag1) + (1|subject),fmri3_pupil_data))
summary(l3<-lmer(scale(pupilDiameter_bc_scaled) ~ scale(convolved_orig) + (1|subject),fmri3_pupil_data))
summary(l4<-lmer(scale(pupilDiameter_bc_scaled) ~ scale(convolved_new) + (1|subject),fmri3_pupil_data))
summary(l5<-lmer(scale(pupilDiameter_scaled) ~ scale(convolved_new) + (1|subject),fmri3_pupil_data))
...
model_data2 <- model.frame(l2)
used_rows2 <- as.numeric(rownames(model_data2))
# Compute residuals for the model
# Get the residuals

# Create a residuals column in processed_data, initializing with NA
fmri3_pupil_data$residuals_lag1 <- NA

residuals_l2 <- residuals(l2)
# Assign residuals to the appropriate rows
fmri3_pupil_data$residuals_lag1[used_rows2] <- residuals_l2
# summary(lmer(scale(luminance_lag1) ~ scale(residuals_lag1) + (1|subject),processed_data))

model_data4 <- model.frame(l4)
used_rows4 <- as.numeric(rownames(model_data4))
fmri3_pupil_data$residuals_conv <- NA

residuals_l4 <- residuals(l4)
# Assign residuals to the appropriate rows
fmri3_pupil_data$residuals_conv[used_rows4] <- residuals_l4

summary(lmer(scale(residuals_conv) ~ scale(convolved_new) + (1|subject),fmri3_pupil_data))

model_data5 <- model.frame(l5)
used_rows5 <- as.numeric(rownames(model_data5))
fmri3_pupil_data$residuals_conv_nobc <- NA

residuals_l5 <- residuals(l5)
# Assign residuals to the appropriate rows
fmri3_pupil_data$residuals_conv_nobc[used_rows5] <- residuals_l5
```
...
ggsave(paste0("~/Desktop/VRMID-analysis/mid-pupil/figures/fig1_emotions.tiff"),dpi = 300, height = 10, width = 10)
```
...
fmri3_data_antprobe <-  fmri3_pupil_data%>%
  ...
  summarise(mean_pupil_bc_scaled = mean(pupilDiameter_bc_scaled, na.rm = T),
            ...
            mean_pupil_bc_scaled_residual_lag = mean(residuals_lag1,na.rm = T),
            mean_pupil_bc_scaled_residual_conv = mean(residuals_conv,na.rm = T),
            mean_pupil_nobc_scaled_residual_conv = mean(residuals_conv_nobc,na.rm = T),
            luminance_mean = mean(luminance,na.rm = T),
            luminance_lag1_mean = mean(luminance_lag1,na.rm = T),
            luminance_convolved_mean = mean(convolved_new,na.rm = T))%>%
  pivot_wider(values_from = c(mean_pupil_bc_scaled,mean_pupil_nobc_scaled,mean_pupil_bc_scaled_residual_lag,mean_pupil_bc_scaled_residual_conv,mean_pupil_nobc_scaled_residual_conv,luminance_mean,luminance_lag1_mean,luminance_convolved_mean), names_from = pupil_window)%>%
  ungroup()%>%
  group_by(subject)%>%
...
tp <- lmer(scale(arousal_scaled) ~  scale(mean_pupil_nobc_scaled_ant) +(1|subject) + (1|cue_size),
              da)
summary(tp)
p_res_lag <- lmer(scale(arousal_scaled) ~  scale(mean_pupil_bc_scaled_residual_lag_ant) +(1|subject) + (1|cue_size),
              da)
p_res_conv <- lmer(scale(arousal_scaled) ~  scale(mean_pupil_bc_scaled_residual_conv_ant) +(1|subject) + (1|cue_size),
              da)
...
tp <- lmer(scale(arousal_scaled) ~  scale(mean_pupil_nobc_scaled_out)+(1|subject) + (1|cue_size),
              do)
summary(tp)
p_res_lag <- lmer(scale(arousal_scaled) ~  scale(mean_pupil_bc_scaled_residual_lag_out) +(1|subject) + (1|cue_size),
              do)
p_res_conv <- lmer(scale(arousal_scaled) ~  scale(mean_pupil_bc_scaled_residual_conv_out) +(1|subject) + (1|cue_size),
              do)
...
for (i in 1:nrow(angles)){
  ...
  data_lm_anti_temp <- da%>%
    mutate(axis1 = valence_scaled * angle_info$cos1 + arousal_scaled * angle_info$sin1,
           axis2 = valence_scaled * angle_info$cos2 + arousal_scaled * angle_info$sin2)
  sum_temp1 <- summary(lmer(scale(axis1) ~ scale(mean_pupil_bc_scaled_residual_ant) + (1|subject) + (1|cue_size),
              data_lm_anti_temp))
  sum_temp2 <- summary(lmer(scale(axis2) ~ scale(mean_pupil_bc_scaled_residual_ant) + (1|subject) + (1|cue_size),
              data_lm_anti_temp))
  lmer_out1 <- rbind(lmer_out1, sum_temp1$coefficients[2,],sum_temp2$coefficients[2,])
}
...
for (i in 1:nrow(angles)){
  ...
  data_lm_out_temp <- do%>%
    mutate(axis1 = valence_scaled * angle_info$cos1 + arousal_scaled * angle_info$sin1,
           axis2 = valence_scaled * angle_info$cos2 + arousal_scaled * angle_info$sin2)
  sum_temp1 <- summary(lmer(scale(axis1) ~ scale(mean_pupil_bc_scaled_residual_out) + (1|subject) + (1|cue_size),
              data_lm_out_temp))
  sum_temp2 <- summary(lmer(scale(axis2) ~ scale(mean_pupil_bc_scaled_residual_out) + (1|subject) + (1|cue_size),
              data_lm_out_temp))
  lmer_out2 <- rbind(lmer_out2, sum_temp1$coefficients[2,],sum_temp2$coefficients[2,])
}
...
ggplot(fmri3_res_axes%>%
  ...
  labs(x = "",
       y = "",
       title = "FMRI study 3 (residual pupil)")+
  theme_blank()+
  theme(legend.position = "none",
...
glm27_array<-as.data.frame(t(read_csv("~/Desktop/midaffemo/glm_maps/glm27_array.csv", col_names = F))) %>% mutate(type = "glm27", tr = row_number())
...
p1a<-ggplot(d1 %>% filter(probe == "anticipation probe"),
  ...
  geom_vline(xintercept = c(10.5,11.5), size = 1, color = "grey")+
  annotate("text",x = 11, y = 0.05, label = "outcome",angle = 90)+
  annotate("text",x = 8, y = 0.05, label = "rating")+
...
summary(l1<-glmer(hit_num ~ glm27 + (1|subject),family = binomial,my_array2__ %>% filter(tr_in_trial == 5) %>% mutate(hit_num = as.factor(hit == "Hit"))))
summary(l2<-lmer(rt ~ glm27 + (1|subject),my_array2__ %>% filter(tr_in_trial == 5, hit == "Hit")))
...
ggsave(paste0("~/Desktop/VRMID-analysis/mid-pupil/figures/similarity_glm27.png"),dpi = 300, height = 6, width = 6)
```
...
p1a_<-ggplot(d1 %>% filter(probe == "anticipation probe"),
  ...
  geom_vline(xintercept = c(10.5,11.5), size = 1, color = "grey")+
  annotate("text",x = 11, y = 0.01, label = "outcome",angle = 90)+
  annotate("text",x = 8, y = 0.01, label = "rating")+
  geom_line(aes(linetype = hit),size = 0.5) +
  geom_point()+
  geom_errorbar(aes(x = tr_in_trial, ymin = mean-se,ymax = mean+se), width = 0.3)+
  scale_color_manual(values = c("navy","red"))+
...
p2b_<-
  ggplot(d2 %>% filter(probe == "outcome probe"),
       aes(x = tr_in_trial, y = mean,color = cue_value))+
  geom_vline(xintercept = c(4.5,5.5), size = 1, color = "grey")+
  annotate("text",x = 5, y = 0.015, label = "fixation",angle = 90)+
  geom_vline(xintercept = c(6.5,7.5), size = 1, color = "grey")+
  annotate("text",x = 7, y =0.015, label = "outcome",angle = 90)+
  annotate("text",x = 10, y = 0.015, label = "rating")+
  geom_line(size = 0.5) +
  geom_point()+
  geom_errorbar(aes(x = tr_in_trial, ymin = mean-se,ymax = mean+se), width = 0.3)+
  scale_color_manual(values = purpleOrange_palette6)+
...
summary(l1<-glmer(hit_num ~ glm33b + (1|subject),family = binomial,my_array2_ %>% filter(tr_in_trial == 5) %>% mutate(hit_num = as.factor(hit == "Hit"))))
summary(l2<-lmer(rt ~ glm33b + (1|subject),my_array2_ %>% filter(tr_in_trial == 5, hit == "Hit")))
...
p1a+p1b+p2a+p2b+plot_layout(nrow = 2)

ggsave(paste0("~/Desktop/VRMID-analysis/mid-pupil/figures/similarity_glm33b.png"),dpi = 300, height = 6, width = 6)
```
...
summary(lmer(scale(glm33b) ~ arousal  + (1|subject) + (1+arousal|probe),my_array2_ %>% filter(tr_in_trial == 5)))
summary(lmer(scale(glm27) ~ arousal  + (1|subject) + (1+arousal|probe),my_array2__ %>% filter(tr_in_trial == 5)))
```
...
ggsave(paste0("~/Desktop/VRMID-analysis/mid-pupil/figures/fig_corr_tc2.tiff"),dpi = 300, height = 5, width =8)

p4a_+labs(title = "pupil GLM")+p4b_

ggsave(paste0("~/Desktop/VRMID-analysis/mid-pupil/figures/fig_corr_tc.tiff"),dpi = 300, height = 3, width =8)
```
