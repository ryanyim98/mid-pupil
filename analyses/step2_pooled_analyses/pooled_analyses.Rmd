...
fig5 <- image_annotate(fig5, "C", size = 55, location = "+50+1500", color = "black", gravity = "northwest")
fig5 <- image_annotate(fig5, "D", size = 55, location = "+50+2300", color = "black", gravity = "northwest")
...
vrmid1_pupil_data_arousal_history <- vrmid1_pupil_data_tonic %>% 
  ...
  mutate(    # Generate recent average win percentages for windows 1 to 20
    ...
    # Generate recent average for pupil_Avg_bc_scaled for lags 0 to 9
    map_dfc(0:9, function(k) {
...
for (l in 0:9){
  l1<-summary(lmer(pupil_size ~ arousal_scaled + (1|Subject),
               vrmid1_pupil_data_arousal_history %>% filter(lag == l, 
                                                          measure == "pupil_Avg_baseline_scaled")))
  l2<-summary(lmer(pupil_size ~ arousal_scaled + (1|Subject),
               vrmid1_pupil_data_arousal_history %>% filter(lag == l, 
                                                          measure == "pupil_Avg_bc_scaled")))
  l3<-summary(lmer(pupil_size ~ arousal_scaled + (1|Subject),
               vrmid1_pupil_data_arousal_history %>% filter(lag == l, 
...
dftemp4 <- dftemp4 %>% 
  mutate(alpha_sig = ifelse(`Pr(>|t|)` < .05/30, 1, 0.2))  # numeric alpha

p4 <- ggplot(dftemp1, aes(x = lag, y = Estimate, color = measure, alpha = alpha_sig)) +
  annotate("rect", xmin = -0.5, ymin = -0.1, xmax = 0.5, ymax = 0.4, alpha = 0.1) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_line(alpha = 0.2) +
  geom_point() +
  ...
  labs(x = "pupil size __ trials ago",
       ...
       color = NULL) +
  theme(legend.position = "top") +
  scale_color_manual(values = c("red4", "grey40", "steelblue")) +
...
d1<-my_array %>% 
         filter(!is.na(hit),
                threshold == 3.5) %>% 
         group_by(tr_in_trial,subject,probe,cue_value) %>% 
         summarise(glmPupil_posonly= mean(glmPupil_posonly,na.rm = T)) %>% 
         group_by(tr_in_trial,cue_value,probe) %>% 
         summarise(mean = mean(glmPupil_posonly,na.rm = T),
                   se = sd(glmPupil_posonly,na.rm = T)/sqrt(n())) %>% 
  filter(tr_in_trial <= 14)
...
ggplot(d1 %>% filter(probe == 1),
  ...
  geom_line(aes(group=cue_value),size = 0.5) +
  geom_point()+
  geom_errorbar(aes(x = tr_in_trial, ymin = mean-se,ymax = mean+se), width = 0.3)+
  labs(y = "Simalirty to pupil template",
       x = "time (s)",
...
d2<-my_array %>% 
         filter(!is.na(hit),
                threshold == 3.5) %>% 
         group_by(tr_in_trial,subject,hit,probe) %>% 
         summarise(glmPupil= mean(glmPupil,na.rm = T)) %>% 
         group_by(tr_in_trial,probe,hit) %>% 
         summarise(mean = mean(glmPupil_posonly,na.rm = T),
                   se = sd(glmPupil_posonly,na.rm = T)/sqrt(n())) %>% 
  filter(tr_in_trial <= 14) %>% 
  mutate(hit = as.factor(hit))
...
ggplot(d2 %>% filter(probe == 1),
  ...
  geom_vline(xintercept = c(4.5,5.5), size = 1, color = "grey")+
  annotate("text",x = 5, y = 0.05, label = "fixation",angle = 90)+
  geom_line(aes(group=hit),size = 0.5) +
  geom_point()+
  geom_errorbar(aes(x = tr_in_trial, ymin = mean-se,ymax = mean+se), width = 0.3)+
  labs(y = "Simalirty to pupil template",
       x = "time (s)",
       subtitle = "anticipation probe",
       title = )+
  scale_color_manual(values = redOrange_palette2)+
  theme(legend.position = "top")+
...
summary(lm(scale(glmArousal) ~ scale(arousal_scaled),my_array__ %>% filter(tr_in_trial == 5, threshold == 1.5)))
```
...
p1m2<-ggplot(d1 %>% filter(probe == "anticipation probe",threshold == 0.5),
       aes(x = tr_in_trial, y = mean))+
  geom_vline(xintercept = c(3.5,4.5,5.5,9.5,10.5,11.5,12.5), size = 1, color = "grey")+
  annotate("text",x = 4, y = 0.05, label = "C")+
  annotate("text",x = 5, y = 0.05, label = "F")+
...
p2m2<-ggplot(d2 %>% filter(threshold == 0.5),
       aes(x = cue_value, y = mean,color = cue_value))+
  geom_point(size = 0.5) +
  geom_point()+
  geom_errorbar(aes(x = cue_value, ymin = mean-se,ymax = mean+se), width = 0.3)+
...
plot_model(l1, type = "pred", terms = "glmPupil")
```
...
summary(lm(scale(glmPupil) ~ scale(arousal_scaled),my_array_ %>% filter(tr_in_trial == 5, threshold == 0.5, probe == "anticipation probe")))
