...
fmri3_data_antprobe <-  fmri3_pupil_data%>%
  ...
  pivot_wider(values_from = c(mean_pupil_bc_scaled,mean_pupil_nobc_scaled,mean_pupil_bc_scaled_residual_lag,mean_pupil_bc_scaled_residual_conv,luminance_mean,luminance_lag1_mean,luminance_convolved_mean), names_from = pupil_window)%>%
  ungroup()%>%
  group_by(subject)%>%
...
tp <- lmer(scale(arousal_scaled) ~  scale(mean_pupil_nobc_scaled_ant) +(1|subject) + (1|cue_size),
              da)
tp_res_lag <- lmer(scale(arousal_scaled) ~  scale(mean_pupil_bc_scaled_residual_lag_ant) +(1|subject) + (1|cue_size),
              da)
tp_res_conv <- lmer(scale(arousal_scaled) ~  scale(mean_pupil_bc_scaled_residual_conv_ant) +(1|subject) + (1|cue_size),
              da)
summary(tp)
summary(tp_res_lag)
summary(tp_res_conv)
...
tp <- lmer(scale(arousal_scaled) ~  scale(mean_pupil_nobc_scaled_out)+(1|subject) + (1|cue_size),
              do)
tp_res_lag <- lmer(scale(arousal_scaled) ~  scale(mean_pupil_bc_scaled_residual_lag_out) +(1|subject) + (1|cue_size),
              do)
tp_res_conv <- lmer(scale(arousal_scaled) ~  scale(mean_pupil_bc_scaled_residual_conv_out) +(1|subject) + (1|cue_size),
              do)
summary(tp)
summary(tp_res_lag)
summary(tp_res_conv)
