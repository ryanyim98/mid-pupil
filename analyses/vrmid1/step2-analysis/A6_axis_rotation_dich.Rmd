...
```{r}
data_lm_anti <- read_csv("~/Desktop/VRMID-analysis/vrmid-pupillometry/data/study1/data_lm_anticipation.csv")%>%
  dplyr::select(-PosA_scaled, -NegA_scaled)

data_lm_outcome <- read_csv("~/Desktop/VRMID-analysis/vrmid-pupillometry/data/study1/data_lm_outcome.csv")%>%
  dplyr::select(-PosA_scaled, -NegA_scaled)%>%
  filter(probe == "out") %>% 
...
```{r}
plot_model(lmer(arousal_scaled ~ valence_scaled * reaction_outcome + (1|Subject),
                data_lm_outcome),
           type = "pred",
...
pb2
ggsave("../../figures/study1_axisrot.png",width = 5, height = 4, units = "in",dpi = 300)
```

```{r}
ggplot()+
  geom_point(data = lmer_out1%>%
         mutate(cos = cos * abs(Estimate),
                sin = sin * abs(Estimate)),
         aes(x = cos, y = sin),
               size = 1, color = "#1B9E77")+
  geom_point(data = lmer_out2%>%
         mutate(cos = cos * abs(Estimate),
                sin = sin * abs(Estimate)),
         aes(x = cos, y = sin),
               size = 1, color = "#D95F02")+
...
pb1
ggsave("../../figures/study1_axisrot_phasic.png",width = 5, height = 4, units = "in",dpi = 300)
```
...
pb3 
ggsave("../../figures/study1_axisrot_tonic.png",width = 5, height = 4, units = "in",dpi = 300)
```
...
ggsave("../../poster/fig9.png",width = 7, height = 2.5, units = "in",dpi = 300)
ggsave("../../figures/study1_axisrot_all.png",width = 11, height = 4, units = "in",dpi = 300)
```
