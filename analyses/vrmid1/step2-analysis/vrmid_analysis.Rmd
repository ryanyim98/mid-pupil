...
```{r}
my.df <-read.csv("../../data/study1/reduced_data.csv")%>%
...
my.df$Time_str <- times(ifelse(hour(hms(as.character(my.df$Time_str)))< 7, 
                               times(my.df$Time_str + "12:00:00"),
                           my.df$Time_str))
...
write_csv(dfOut,"/Users/rh/Desktop/VRMID-analysis/data/vrmid-pupillometry/study1/per_second_data.csv")
```
...
ggsave("../../poster/fig3.png",width = 10, height = 3, units = "in",dpi = 300)
```
...
lm7d <- lmer(pupil_size_scaled ~ condition * trial.type + (1|Subject),
...
p1+p2+p3
ggsave("../../poster/fig_prereg1.png",width = 9, height = 2.5, units = "in",dpi = 300)
```
...
ggsave("../../figures/space-ant.png",width = 5.5, height = 4, units = "in",dpi = 300)
```
...
ggplot(data = aff_traj_avg_noRotat%>%filter(report_type %in% c("anticipation","Hit")), aes(x = valence_mean, y = arousal_mean, color = trial.type, group = report_type))+
  ...
  geom_segment(aes(y=arousal_mean, yend=arousal_mean,
               ...
               width=.5)+
  scale_color_manual(values = purpleOrange_palette6)+
  geom_line(aes(group = trial.type, color = trial.type), linetype = "dotted")+
  labs(title = "Affect circumplex",
       x = "valence",
       y = "arousal",
       color = "cue value")+
  theme_blank()+
  xlim(1,5)
ggsave("../../figures/space-hit.png",width = 5.5, height = 4, units = "in",dpi = 300)
```
...
ggsave("../../figures/space-miss.png",width = 5.5, height = 4, units = "in",dpi = 300)
```
