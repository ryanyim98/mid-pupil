...
#                 df_axes1$cos[1] * valence_scaled + df_axes1$sin[1] * arousal_scaled,
#                 ifelse(probe == "out",
#                        df_axes1$cos[2] * valence_scaled + df_axes1$sin[2] * arousal_scaled,
#                        NA)))
...
ggplot(data_pupil %>% 
         filter(probe == "anti",sample_in_trial_t < 20) %>% 
         group_by(sample_in_trial_t) %>% 
         summarise(pupil_Avg = mean(pupil_Avg,na.rm = T)), 
       aes(x = sample_in_trial_t, y = pupil_Avg))+
...
write.csv(regression_coefs_ant_bc, "../../data/study1/ant_regression_coefs_bc.csv")
```
...
write.csv(regression_coefs_ant_nobc, "../../data/study1/ant_regression_coefs_nobc.csv")
```

#get the peak of the raw signal
```{r}
regression_coefs_ant_nobc <- read_csv("../../data/study1/ant_regression_coefs_nobc.csv")
regression_coefs_ant_nobc_smoothed <- regression_coefs_ant_nobc %>% 
  mutate(smoothed_tval = zoo::rollmean(`t value`, k = 120, fill = NA))

ggplot(regression_coefs_ant_nobc_smoothed, aes(x = sample_point,y = smoothed_tval))+
  ...
  #   annotate("rect", xmin =15.5*120, xmax = 17.5*120, ymin = 1, ymax = 15,
  #          alpha = .2, color = "violet", fill = NA, linetype = "dashed")+
  # annotate("rect", xmin =17.5*120, xmax = 19.5*120, ymin = 1, ymax = 15,
...
ggsave("../../figures/study1_window_raw_ant.png",width = 6, height = 4, units = "in",dpi = 300)
```
...
write.csv(regression_coefs_out_bc, "../../data/study1/out_regression_coefs_bc.csv")
```
...
for (t in 1:20) {
  ...
  if (nrow(df_out_temp) > 0){
     lm_out_temp <- lmer(scale(arousal_scaled) ~ scale(pupil_Avg_scale) + (1|Subject) + (1|condition),df_out_temp)
...
write.csv(regression_coefs_out_nobc, "../../data/study1/out_regression_coefs_nobc.csv")
```
...
pp1/pp2
ggsave("../../figures/study1_corrpeaks.png",width = 6, height = 10, units = "in",dpi = 300)
```
...
ggplot(regression_coefs_out_bc_smoothed, aes(x = sample_point,y = smoothed_tval))+
  ...
  annotate("rect", xmin = 5.5*120, xmax = 7.5*120, ymin = -1, ymax = 8,
           alpha = .2, color = "orange", fill = NA, linetype = "dashed")+
    annotate("rect", xmin =7.5*120, xmax = 9.5*120, ymin = -1, ymax = 8,
           alpha = .2, color = "violet", fill = NA, linetype = "dashed")+
  annotate("rect", xmin =9.5*120, xmax = 11.5*120, ymin = -1, ymax = 8,
           alpha = .2, color = "blue", fill = NA, linetype = "dashed")+
  scale_x_continuous(breaks = seq(0,2400,120), labels =  0:20)+
  labs(x = "time (sec)",
       y = "Correlation strength (t value)",
...
ggsave("../../figures/study1_window_phasic_out.png",width = 6, height = 4, units = "in",dpi = 300)
```
...
```{r}
p1b + p2b
ggsave("../../figures/study1_phasiccluster.png",width = 12, height = 4, units = "in",dpi = 300)
```
...
ggsave("../../figures/study1_window_all_ant.png",width = 6, height = 4, units = "in",dpi = 300)
...
ggsave("../../figures/study1_window_all_out.png",width = 6, height = 4, units = "in",dpi = 300)
```
